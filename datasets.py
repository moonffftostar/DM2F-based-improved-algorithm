...
def make_dataset_HazeRD(root):
    #hazy_distance = ['50','100','200','500','1000']
    hazy_distance = ['500']
    items = []
    for img_name in os.listdir(os.path.join(root,'data','img')):
        img, idx, x = os.path.splitext(img_name)[0].split('_')
        for dis in hazy_distance:
            haze = os.path.join(root,'data','simu',img + '_' +idx + '_' + dis +'.jpg')
            gt = os.path.join(root,'data','img',img_name)
...
class HazeRdDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        haze_path,gt_path = self.imgs[index]
        name = os.path.splitext(os.path.split(haze_path)[1])[0][4:]

        haze = Image.open(haze_path).convert('RGB')
        #haze = haze.resize((620,460),resample=0)
        haze = to_tensor(haze)

        gt = Image.open(gt_path).convert('RGB')
        #gt = gt.resize((620,460),resample=0)
        gt = to_tensor(gt)
...
if __name__ == '__main__':
    #make_dataset_HazeRD('./data/RESIDE/HazeRD')
    data = HazeRdDataset(root='./data/RESIDE/HazeRD')
    data = data[0]